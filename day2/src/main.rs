...
impl Game {
    ...
    fn can_get_power(&self) -> bool {
        if self.max_green.is_some() && self.max_red.is_some() && self.max_blue.is_some() {
            return true
        }
        false
    }
    ...
    fn get_power(&self) -> i32 {
        if !self.can_get_power() {
            panic!("Could not get power of max set");
        } else {
            return self.max_green.unwrap() * self.max_red.unwrap() * self.max_blue.unwrap()
        }
    }
}
...
fn main() {
    ...
    let mut set_powers: Vec<i32> = vec![];

    for mut game in games {
        game.find_max();
        set_powers.push(game.get_power());
    }

    println!("{}", set_powers.iter().sum::<i32>())
}
